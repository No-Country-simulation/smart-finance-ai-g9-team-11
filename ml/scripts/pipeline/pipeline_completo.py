...
from ml.scripts.algorithm2.dataset_profile import FEATURES_MODELO_PERFIL
from ml.scripts.recomendacoes_engine import gerar_recomendacoes

# Este arquivo está em: ml/scripts/pipeline/pipeline_completo.py
# parents[0] = ml/scripts/pipeline
# parents[1] = ml/scripts
# parents[2] = ml   <- é aqui que fica a pasta "models"
    ...
    if _model is None:
        if not MODEL_PATH.exists():
            raise FileNotFoundError(
                ...
                "Verifique se os artefatos (.pkl) estão em ml/models/."
            )
